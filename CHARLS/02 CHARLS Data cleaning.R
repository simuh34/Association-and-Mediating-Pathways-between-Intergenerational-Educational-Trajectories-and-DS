...
setwd('D:\\R project\\Educational mobility and depression\\CHARLS')
df <- read.csv("charls.csv")

# Data filtering
df <- df %>%
  filter(inw10 == 1, age1 >= 50) %>%
  filter(rowSums(is.na(across(c("cesd2014","cesd2016","cesd2018")))) < 3) %>%
...
df <- df %>%
  ...
  # Birth decade grouping & cognitive impairment
  ...
  mutate(
    ...
    cognitive_impairment = ifelse(orient < mean(orient, na.rm=TRUE)-1.5*sd(orient, na.rm=TRUE) & r1tr20 < mean(r1tr20, na.rm=TRUE)-1.5*sd(r1tr20, na.rm=TRUE), "Impaired", "Normal")
  ) %>%
  ungroup() %>%
  # Educational mobility (residual + categorical)
  mutate(
    educational_mobility_residual = resid(lm(percentile_rank ~ percentile_rank_parent)),
...
df <- df %>%
  mutate(across(starts_with("cesd"), ~ifelse(is.na(.), NA, ifelse(. >= 10, 1, 0))))
